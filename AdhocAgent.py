...
class AdhocAgent(object):
    """
        ...
        The training algorithm is based on the RL^2 algorithm.
        Unlike the latest AHT algorithms, this method simply does not have a decoder
        that predicts others' actions.
    """
    def __init__(self, obs_size, configs, act_sizes, device, logger):
        """
        ...
        policy_dimensions = [
            obs_size-self.act_size+self.config.model["agent_rep_size"], *self.config.model.actor_dims, self.act_size
        ]
        ...
        # +1 for additional rewards that is input to the encoder
        
        encoder_dimensions = [
            obs_size+1, 
            *self.config.model.enc_dims, 
            self.config.model["agent_rep_size"]
        ]
        ...
        crit_dimensions = [
            obs_size - self.act_size + self.config.model["agent_rep_size"], *self.config.model.critic_dims, 1
        ]
    ...
    def get_teammate_representation(self, obs, prev_cell):
        ...
        input = torch.tensor(obs).double().to(self.device)[:, -1, :]
    ...
    def decide_acts(self, obs_w_commands, eval=False):
        """
    ...
    # TODO incorporate trunc
    def compute_critic_loss(self, obs_batch, rewards_batch, done_batch, trunc_batch, n_obs_batch, representations):
        """
            A method that computes the AHT agent's critic loss.
        """
        observation = obs_batch[:,:,-1,:-self.act_size]
        n_observation = n_obs_batch[:,:,-1,:-self.act_size]
        ...
        for t_id in reversed(range(update_length)):
            if t_id == update_length-1:
                bootstrapped_target = self.target_action_value_function(target_critic_input[:,t_id,:])
            else:
                bootstrapped_target[trunc_batch[:,t_id]==1] = self.target_action_value_function(target_critic_input[:,t_id,:])[trunc_batch[:,t_id]==1]
            ...
            target_value.append(
                bootstrapped_target.detach().clone()
            )
    ...
    def compute_actor_and_entropy_loss(self, obs_batch, acts_batch, rewards_batch, done_batch, trunc_batch, n_obs_batch, representations):
        """
        ...
        for t_id in reversed(range(update_length)):
            bootstrap_input = torch.cat([n_observation[:, t_id, :], detached_representations[:, t_id, :]], dim=-1)
            if t_id == update_length-1:
                bootstrapped_target = self.target_action_value_function(bootstrap_input)
            else:
                bootstrapped_target[trunc_batch[:,t_id]==1] = self.target_action_value_function(bootstrap_input)[trunc_batch[:,t_id]==1]
            
            bootstrapped_target = rewards_batch[:, t_id].unsqueeze(-1) + self.config.train["gamma"] * \
                                  (1 - done_batch[:, t_id].unsqueeze(-1)) * bootstrapped_target

            predicted_value.append(
                bootstrapped_target.clone()
            )
        ...
        print(act_log_likelihood.size(), all_predicted_values.size(), baseline_values.size())
    ...
    def update(self, batches, agent_representations):
        ...
        trunc_batch = torch.tensor(batches[4]).double().to(self.device)
        rewards_batch = torch.tensor(batches[5]).double().to(self.device)[:, :, -1]
        ...
        total_actor_loss, total_entropy_loss = self.compute_actor_and_entropy_loss(
            obs_batch, acts_batch, rewards_batch, done_batch, trunc_batch, n_obs_batch, agent_representations
        )
