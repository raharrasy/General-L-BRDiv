...
class EpisodicExperienceReplay(object):
    ...
    def __init__(self, ob_shape, act_shape, max_episodes=100000, max_eps_length=20):
        ...
        num_agents = obs_shape[-2]

        self.obs = np.zeros(obs_shape)
        self.actions = np.zeros(acts_shape)
        self.next_obs = np.zeros(obs_shape)
        self.termination = np.zeros([max_episodes, max_eps_length])
        self.truncation = np.zeros([max_episodes, max_eps_length])
        self.rewards = np.zeros([max_episodes, max_eps_length, num_agents])
    ...
    def add_episode(self, obses, acts, rewards, termination, truncation, next_obses):
    #def add_episode(self, obses, acts, rewards, total_rew, eps_length, gamma):
        ...
        self.termination[self.pointer, :eps_length] = termination[:eps_length]
        self.truncation[self.pointer, :eps_length] = truncation[:eps_length]
    ...
    def sample_all(self):
        ...
        """
        return self.obs, self.actions, self.next_obs, self.termination, self.truncation, self.rewards
    ...
    def save(self, dir_location):
        ...
        with open(dir_location+'/termination.npy', 'wb') as f:
            np.save(f, self.termination)
        with open(dir_location+'/truncation.npy', 'wb') as f:
            np.save(f, self.truncation)
    ...
    def load(self, dir_location):
        ...
        self.termination = np.load(dir_location + "/termination.npy")
        self.truncation = np.load(dir_location + "/truncation.npy")
