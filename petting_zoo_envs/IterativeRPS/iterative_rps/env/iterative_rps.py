...
def raw_env(render_mode=None):
    ...
    """
    env = parallel_rps_env(render_mode=render_mode)
...
class parallel_env(ParallelEnv):
    ...
    metadata = {"render_modes": ["human"], "name": "iter_rps_v1"}
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = {agent: np.argmax(observations[agent][:4]) for agent in observations}
    ...
    def step(self, actions):
        ...
        for i in range(len(self.agents)):
            obs_vec = np.zeros(8)
            obs_vec[int(actions[self.agents[1 - i]])] = 1
            obs_vec[int(actions[self.agents[i]])+4] = 1
